...
class DemoActionType(str, Enum):
    ...
    WAIT_FOR_NAVIGATION_IDLE = "wait_for_navigation_idle"
    ...
    ASSERT_NOT_TEXT_VISIBLE = "assert_not_text_visible"
    ASSERT_ELEMENT_VISIBLE = "assert_element_visible"
    ASSERT_NOT_ELEMENT_VISIBLE = "assert_not_element_visible"
...
class DemoScript:
    ...
    def _validate_step(self, index: int, step: DemoStep) -> None:
        ...
        if step.action == DemoActionType.WAIT_FOR_NOT_TEXT_VISIBLE:
            if step.text is None:
                raise ValueError(f"Step {index}: wait_for_not_text_visible requires text")
            if step.seconds is not None and step.seconds < 0:
                raise ValueError(f"Step {index}: wait_for_not_text_visible requires non-negative seconds")

        if step.action == DemoActionType.WAIT_FOR_NAVIGATION_IDLE:
            if step.seconds is not None and step.seconds < 0:
                raise ValueError(f"Step {index}: wait_for_navigation_idle requires non-negative seconds")
        ...
        if step.action == DemoActionType.ASSERT_NOT_TEXT_VISIBLE and step.text is None:
            raise ValueError(f"Step {index}: assert_not_text_visible requires text")
        ...
        if step.action == DemoActionType.ASSERT_NOT_ELEMENT_VISIBLE and not step.selector:
            raise ValueError(f"Step {index}: assert_not_element_visible requires selector")
