...
function selectSegment(track,segment){selected={track,segment};const t=snapshot.project.timeline.tracks[track],s=t.segments[segment];document.querySelector('#segment-editor').hidden=false;document.querySelector('#segment-title').textContent=t.type+' · segment '+(segment+1);document.querySelector('#segment-start').value=s.start_seconds;document.querySelector('#segment-end').value=s.end_seconds;render()}
document.querySelector('#apply-segment').addEventListener('click',()=>{if(!selected)return;const start=Number(document.querySelector('#segment-start').value),end=Number(document.querySelector('#segment-end').value);const status=document.querySelector('#status');if(!Number.isFinite(start)||!Number.isFinite(end)||start<0||end<=start){status.textContent='Segment end must be greater than start';return}const s=snapshot.project.timeline.tracks[selected.track].segments[selected.segment];s.start_seconds=start;s.end_seconds=end;render();status.textContent='Unsaved segment changes'});
