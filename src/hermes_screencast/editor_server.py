...
function selectSegment(track,segment){selected={track,segment};const t=snapshot.project.timeline.tracks[track],s=t.segments[segment],camera=t.type==='camera.zoom',cursor=t.type==='cursor.motion',annotation=t.type==='annotation.overlay';document.querySelector('#segment-editor').hidden=false;document.querySelector('#segment-title').textContent=t.type+' · segment '+(segment+1);document.querySelector('#segment-start').value=s.start_seconds;document.querySelector('#segment-end').value=s.end_seconds;document.querySelector('#camera-fields').hidden=!camera;document.querySelector('#cursor-fields').hidden=!cursor;document.querySelector('#annotation-fields').hidden=!annotation;if(camera){document.querySelector('#zoom-scale').value=s.scale;document.querySelector('#focus-x').value=s.focus.x;document.querySelector('#focus-y').value=s.focus.y}if(cursor){const target=t.anchors.find(a=>a.source_event_sequence===s.source_event_sequences[1]);document.querySelector('#cursor-arrival').textContent=(target?.action||'move')+' · '+s.arrival_seconds+' s';document.querySelector('#cursor-from-x').value=s.from.x;document.querySelector('#cursor-from-y').value=s.from.y;document.querySelector('#cursor-to-x').value=s.to.x;document.querySelector('#cursor-to-y').value=s.to.y;document.querySelector('#cursor-control-1-x').value=s.control_1.x;document.querySelector('#cursor-control-1-y').value=s.control_1.y;document.querySelector('#cursor-control-2-x').value=s.control_2.x;document.querySelector('#cursor-control-2-y').value=s.control_2.y}if(annotation){const text=s.kind==='text',bounds=s.kind==='box'||s.kind==='highlight',arrow=s.kind==='arrow';document.querySelector('#annotation-kind').textContent=s.kind;document.querySelector('#annotation-color').value=s.style.color;document.querySelector('#annotation-opacity').value=s.style.opacity;document.querySelector('#annotation-text-fields').hidden=!text;document.querySelector('#annotation-bounds-fields').hidden=!bounds;document.querySelector('#annotation-arrow-fields').hidden=!arrow;if(text){document.querySelector('#annotation-text').value=s.text;document.querySelector('#annotation-x').value=s.position.x;document.querySelector('#annotation-y').value=s.position.y;document.querySelector('#annotation-font-size').value=s.style.font_size}if(bounds){document.querySelector('#bounds-x').value=s.bounds.x;document.querySelector('#bounds-y').value=s.bounds.y;document.querySelector('#bounds-width').value=s.bounds.width;document.querySelector('#bounds-height').value=s.bounds.height}if(arrow){document.querySelector('#arrow-from-x').value=s.from.x;document.querySelector('#arrow-from-y').value=s.from.y;document.querySelector('#arrow-to-x').value=s.to.x;document.querySelector('#arrow-to-y').value=s.to.y}}render()}
document.querySelector('#apply-segment').addEventListener('click',()=>{if(!selected)return;const start=Number(document.querySelector('#segment-start').value),end=Number(document.querySelector('#segment-end').value),status=document.querySelector('#status'),track=snapshot.project.timeline.tracks[selected.track],s=track.segments[selected.segment],canvas=snapshot.project.composition.canvas;if(!Number.isFinite(start)||!Number.isFinite(end)||start<0||end<=start){status.textContent='Segment end must be greater than start';return}if(track.type==='camera.zoom'){const scale=Number(document.querySelector('#zoom-scale').value),x=Number(document.querySelector('#focus-x').value),y=Number(document.querySelector('#focus-y').value);if(!Number.isFinite(scale)||scale<1||!pointFits(x,y,canvas)){status.textContent='Camera scale and focus must fit the canvas';return}s.scale=scale;s.focus={x,y}}if(track.type==='cursor.motion'){const from={x:Number(document.querySelector('#cursor-from-x').value),y:Number(document.querySelector('#cursor-from-y').value)},to={x:Number(document.querySelector('#cursor-to-x').value),y:Number(document.querySelector('#cursor-to-y').value)},control1={x:Number(document.querySelector('#cursor-control-1-x').value),y:Number(document.querySelector('#cursor-control-1-y').value)},control2={x:Number(document.querySelector('#cursor-control-2-x').value),y:Number(document.querySelector('#cursor-control-2-y').value)},previous=track.segments[selected.segment-1],next=track.segments[selected.segment+1];if(end>s.arrival_seconds||(previous&&start<previous.end_seconds)||(next&&end>next.start_seconds)||![from,to,control1,control2].every(p=>pointFits(p.x,p.y,canvas))){status.textContent='Cursor timing and path must fit the canvas';return}syncCursorEndpoints(track,s,from,to);s.control_1=control1;s.control_2=control2}if(track.type==='annotation.overlay'){const color=document.querySelector('#annotation-color').value.trim(),opacity=Number(document.querySelector('#annotation-opacity').value);if(!/^#[0-9a-f]{6}([0-9a-f]{2})?$/i.test(color)||!Number.isFinite(opacity)||opacity<0||opacity>1){status.textContent='Annotation color or opacity is invalid';return}if(s.kind==='text'){const text=document.querySelector('#annotation-text').value.trim(),x=Number(document.querySelector('#annotation-x').value),y=Number(document.querySelector('#annotation-y').value),fontSize=Number(document.querySelector('#annotation-font-size').value);if(!text||!pointFits(x,y,canvas)||!Number.isFinite(fontSize)||fontSize<=0){status.textContent='Text annotation must fit the canvas';return}s.text=text;s.position={x,y};s.style.font_size=fontSize}else if(s.kind==='box'||s.kind==='highlight'){const x=Number(document.querySelector('#bounds-x').value),y=Number(document.querySelector('#bounds-y').value),width=Number(document.querySelector('#bounds-width').value),height=Number(document.querySelector('#bounds-height').value);if(!boundsFit(x,y,width,height,canvas)){status.textContent='Annotation bounds must fit the canvas';return}s.bounds={x,y,width,height}}else if(s.kind==='arrow'){const fromX=Number(document.querySelector('#arrow-from-x').value),fromY=Number(document.querySelector('#arrow-from-y').value),toX=Number(document.querySelector('#arrow-to-x').value),toY=Number(document.querySelector('#arrow-to-y').value);if(!pointFits(fromX,fromY,canvas)||!pointFits(toX,toY,canvas)){status.textContent='Annotation arrow must fit the canvas';return}s.from={x:fromX,y:fromY};s.to={x:toX,y:toY}}s.style.color=color.toUpperCase();s.style.opacity=opacity}s.start_seconds=start;s.end_seconds=end;render();status.textContent='Unsaved segment changes'});
function syncCursorEndpoints(track,segment,from,to){const [fromSequence,toSequence]=segment.source_event_sequences;for(const anchor of track.anchors){if(anchor.source_event_sequence===fromSequence)anchor.position={...from};if(anchor.source_event_sequence===toSequence)anchor.position={...to}}for(const item of track.segments){const [first,last]=item.source_event_sequences;if(first===fromSequence)item.from={...from};if(last===fromSequence)item.to={...from};if(first===toSequence)item.from={...to};if(last===toSequence)item.to={...to}}}
